...
-- relatório 1 --
SELECT 
    emp.nome AS "Nome Empregado",
    emp.cpf AS "CPF Empregado",
    emp.data_admissao AS "Data Admissão",
...
FROM 
    empregado emp
JOIN 
    telefone tel ON emp.cpf = tel.empregado_cpf
JOIN 
    departamento dep ON emp.departamento_idDepartamento = dep.idDepartamento
WHERE 
    emp.data_admissao BETWEEN '2019-01-01' AND '2022-03-31'
ORDER BY 
    emp.data_admissao DESC;
...
FROM 
    departamento dep
LEFT JOIN 
    empregado emp ON dep.idDepartamento = emp.departamento_idDepartamento
GROUP BY 
    dep.nome
ORDER BY 
    dep.nome;
...
SELECT 
    ...
    emp.sexo AS "Sexo",
    emp.salario AS "Salário",
    COUNT(fc.idFormaPagCompra) AS "Quantidade Vendas",
    SUM(fc.valorpago) AS "Total Valor Vendido",
    SUM(emp.comissao) AS "Total Comissão das Vendas"
FROM 
    empregado emp
LEFT JOIN 
    formapagcompra fc ON emp.cpf = fc.empregado_cpf
GROUP BY 
    emp.cpf
ORDER BY 
    COUNT(fc.idFormaPagCompra) DESC;
...
SELECT 
    ...
    emp.sexo AS "Sexo",
    emp.salario AS "Salário",
    COUNT(s.idServico) AS "Quantidade Vendas com Serviço",
    SUM(s.valor) AS "Total Valor Vendido com Serviço",
    SUM(emp.comissao) AS "Total Comissão das Vendas com Serviço"
FROM 
    empregado emp
LEFT JOIN 
    venda v ON emp.cpf = v.empregado_cpf
LEFT JOIN 
    servico s ON v.idVenda = s.venda_idVenda
WHERE 
    s.idServico IS NOT NULL
GROUP BY 
    emp.cpf
ORDER BY 
    COUNT(s.idServico) DESC;

-- relatório 6 --
SELECT 
    p.nome AS "Nome do Pet",
    s.data AS "Data do Serviço",
    ser.nome AS "Nome do Serviço",
...
FROM 
    servico s
INNER JOIN 
    pet p ON s.pet_idPet = p.idPet
INNER JOIN 
    servico_tem_servico sts ON s.idServico = sts.servico_idServico
INNER JOIN 
    servico ser ON sts.servico_idServico2 = ser.idServico
INNER JOIN 
    empregado emp ON s.empregado_cpf = emp.cpf
ORDER BY 
    s.data DESC;
...
FROM 
    venda v
INNER JOIN 
    empregado emp ON v.empregado_cpf = emp.cpf
ORDER BY 
    v.data DESC;
...
SELECT 
    ...
    COUNT(vs.servicos_idServico) AS "Quantidade Vendas",
    SUM(vs.valor) AS "Total Valor Vendido"
FROM 
    venda_servico vs
INNER JOIN 
    servico s ON vs.servicos_idServico = s.idServico
GROUP BY 
    s.nome
ORDER BY 
    COUNT(vs.servicos_idServico) DESC
LIMIT 10;

-- relatório 9 --
SELECT 
    fpc.tipo AS "Tipo Forma Pagamento",
    COUNT(vc.formapagcompra_idFormaPagCompra) AS "Quantidade Vendas",
    SUM(vc.valor) AS "Total Valor Vendido"
FROM 
    vendacompra vc
INNER JOIN 
    formapagcompra fpc ON vc.formapagcompra_idFormaPagCompra = fpc.idFormaPagCompra
GROUP BY 
    fpc.tipo
ORDER BY 
    COUNT(vc.formapagcompra_idFormaPagCompra) DESC;

-- relatório 10 --
SELECT 
    vc.dataVenda AS "Data Venda",
    COUNT(vc.idVendaCompra) AS "Quantidade de Vendas",
    SUM(vc.valor) AS "Valor Total Venda"
FROM 
    vendacompra vc
GROUP BY 
    vc.dataVenda
...
SELECT 
    ...
    p.valorProduto AS "Valor Produto",
...
SELECT 
    ...
    SUM(iv.valorVenda) AS "Valor Total Recebido pela Venda do Produto"
FROM 
    produto p
LEFT JOIN 
    itemvenda iv ON p.idProduto = iv.produto_idProduto
LEFT JOIN 
    venda v ON iv.venda_idVenda = v.idVenda
GROUP BY 
    p.nomeProduto
ORDER BY 
    COUNT(iv.produto_idProduto) DESC;
